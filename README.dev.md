...
> 記事ごとに OGP 背景を変えたい場合は `public/files/<記事ID>/_thumbnail.png` (1200×630) を配置するか、`pnpm image` の Web エディタで作成できます。詳細は [README.writer.md](./README.writer.md#サムネイルogp画像のカスタマイズ) を参照。
...
```bash
pnpm image                  # article/<id> ブランチ上で実行すると自動で対象記事を判定
pnpm image <記事ID>          # 明示指定
```
